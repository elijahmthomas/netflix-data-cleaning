...
# 3. Initial Cleaning / Standardizes Column Names / Fills Missing Values with 'Unknown'
...
# Format the country column further
df['country'] = (
    df['country']
    .astype(str)
...
# Define your known clean list / e.g., list of acceptable names
...
def map_strict_genres(listed_str):
    ...
    return list(matched) if matched else ['Other']    # Further cleaning done in Power Query due to the range of listed categories by Netflix

# Apply to dataset
df['broad_genres'] = df['listed_in'].apply(map_strict_genres)

df = df[~df.apply(lambda row: row.astype(str).str.contains('Unknown')).any(axis=1)] # This removes rows with 'Unknown' in any column, this is not needed for datasets, I used this to simplify the dashboard for now. Changes can be applied later 

# 6. Clean Duration & Convert to Minutes
df['duration_minutes'] = df['duration'].str.extract(r'(\d+)').astype(float)

# 7. Group by 30-Minute Bins / (e.g., "60-89 min", "90-119 min")
...
# 8. Count and Export Cleaned Data / This adds a simple 'count' column used in many graphs
df['count'] = 1
df.to_csv('netflix_cleaned.csv', index=False)

# 9. Output Summary / this is optional, but is nice for clarifying the product
